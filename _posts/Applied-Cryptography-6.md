...
接著我們看步驟二的c，由於{% math %} c \ \ \equiv \ \ z^Q  \ \ (mod \ \ p){% endmath %}，因此{% math %} c^{2^S} \ \ \equiv \ \ (z^Q)^{2^S} \ \ \equiv \ \ z^{Q2^S} \ \ \equiv \ \ z^{p-1} \ \ \equiv \ \ 1 \ \ (mod \ \ p) {% endmath %}，此時還不確定$2^S$就是c的order，因此我們檢查$2^{S-1}$：{% math %} c^{2^{S-1}} \ \ \equiv \ \ z^{ \frac{p-1}{2}} \ \ -1 \ \ (mod \ \ p){% endmath %}，最後的等於-1是Legendre symbol，由此可知c的order為$2^S$。
...
接著我們看一下c'，{% math %} (c')^{2^{S'}} \ \ \equiv \ \ b^{2^{S'} \ \ * \ \ 2} \ \ \equiv \ \ b^{2^{S'+1}} \ \ \equiv \ \ c^{2^{S-S'-1+S'+1}}  \ \ \equiv \ \ c^{2^S} \ \ \equiv \ \ 1 \ \ (mod \ \ p){% endmath %}，這邊就可以了解S-S'-1的-1是為了消掉後面的平方，-S'是為了後來要讓$2^{S'}$消掉，而{% math %} (c')^{2^{S'-1}} \ \ \equiv \ \ c^{2^{S-1}} \ \ \equiv \ \ -1 \ \ (mod \ \ p) {% endmath %}，故c'的order為$2^{S'}$。由此可知下一輪的t'會有order S'' < S'，所以當最後的S''為0時，t就是1了，此時的R即為根號解。
